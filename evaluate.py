...
for threshold in thresholds:
    anomaly_predictions = [1 if score > threshold else 0 for score in anomaly_score_total_list]
    f1 = f1_score(labels_list, anomaly_predictions)
    precision = precision_score(labels_list, anomaly_predictions)
    recall = recall_score(labels_list, anomaly_predictions)
    
    # Atualizar se houver uma melhora em qualquer métrica
    if f1 > best_f1:
        best_f1 = f1
...
print('Precision: ', best_precision*100, '%')
print('Recall: ', best_recall*100, '%')
print('F1-Score: ', best_f1*100, '%')
